  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      ...
      <section className="panel p-4">
        ...
        <p className="muted mb-3 text-[11px]">
          Im Prototyp nur dargestellt. Für den Echtbetrieb werden API-Token und Secret des
          API-Benutzers benötigt; die Authentifizierung läuft über HMAC v2.
        </p>
        ...
        <ul className="space-y-1.5 text-[13px]">
          ...
          <StatusRow
            ...
            state="bereit"
            note="uploadfile → tmpUploadId, dann module=task mit relatedRecordId"
          />
          <StatusRow label="Aufgaben-Datei löschen" state="bereit" note="delete auf fileRelation, relationtype=task" />
          <StatusRow
            label="Aufgaben-Dateien lesen / herunterladen"
            state="offen"
            note="kein dokumentierter Call – nur estate und address haben ihn"
          />
        </ul>
...
function StatusRow({ label, state, note }: { label: string; state: "bereit" | "offen" | "prüfen"; note: string }) {
  const style =
    state === "bereit"
